...
function renderPortfolioVenn(containerId) {
  const card = makeChartCard('venn-chart', 'Owner Identity Segments');
  document.getElementById(containerId).appendChild(card);

  const V = CONFIG.venn;
  _charts['venn-chart'] = new Chart(document.getElementById('venn-chart'), {
    type: 'venn',
    data: {
      labels: ['Single-community', 'Multi-community', 'Delinquent'],
      datasets: [{
        label: 'Owner Segments',
        data: [
          { sets: ['Single-community'],                              value: V.aOnly,  label: fmt(V.aOnly)  },
          { sets: ['Multi-community'],                              value: V.bOnly,  label: fmt(V.bOnly)  },
          { sets: ['Delinquent'],                                   value: 0,        label: ''            },
          { sets: ['Single-community', 'Delinquent'],               value: V.aPlusC, label: fmt(V.aPlusC) },
          { sets: ['Multi-community', 'Delinquent'],                value: V.bPlusC, label: fmt(V.bPlusC) },
          { sets: ['Single-community', 'Multi-community'],          value: 0,        label: ''            },
          { sets: ['Single-community', 'Multi-community', 'Delinquent'], value: 0,   label: ''            },
        ],
        backgroundColor: [
          CONFIG.palette.slate + '55',
          CONFIG.palette.blue  + '55',
          CONFIG.palette.coral + '22',
          CONFIG.palette.coral + '55',
          CONFIG.palette.red   + 'AA',
          'transparent',
          'transparent',
        ],
        borderColor: [
          CONFIG.palette.slate,
          CONFIG.palette.blue,
    ...
    options: {
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => {
              if (ctx.raw.sets.length === 2 && ctx.raw.sets.includes('Multi-community') && ctx.raw.sets.includes('Delinquent')) {
                return ['Cross-portfolio risk: ' + fmt(CONFIG.venn.bPlusC) + ' entities',
                        'Delinquent in 1+ community, active in others.',
                        'CiraNet cannot surface this today.'];
              }
              return fmt(ctx.raw.value) + ' owners';
            },
          },
        },
      },
...
function renderPortfolioTab() {
  ...
  renderPortfolioVenn('tab2-grid1');
